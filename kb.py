...
async def menu_keyboard(cur_lang):
    ...
    if cur_lang == 'ru': 
        ...
        main_menu.add(types.InlineKeyboardButton('Сменить голос', callback_data='voice'))
        ...
        main_menu = types.InlineKeyboardMarkup(row_width=5)
        for mode in modes:
            main_menu.add(types.InlineKeyboardButton(f'Turn on "{mode["verbose_name_en"]}" mode', callback_data=f'change_mode_{mode["name"]}'))
        main_menu.add(types.InlineKeyboardButton('Subscribe', callback_data='subscribe'))
        main_menu.add(types.InlineKeyboardButton('Change voice', callback_data='voice'))
